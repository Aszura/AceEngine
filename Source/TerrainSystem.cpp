...
TerrainSystem::TerrainSystem(EntityWorld* entityWorld)
...
void TerrainSystem::start()
{
	for (auto& terrainComp : mEntityWorld->getTerrainWorld().getArray())
	{
		...
		float* heightData = new float[heightMapSize.x * heightMapSize.y];

		std::vector<unsigned int>& terrainIndices = terrainComp.terrainIndices;
		terrainIndices.reserve(heightMapSize.x*(heightMapSize.y - 1) * 2 + (heightMapSize.y - 1));
		...
		//	terrainIndices.push_back(heightMapSize.x*heightMapSize.y); //Add primitive restart
		//}
		...
		terrainComp.indicesNum = terrainIndices.size();
