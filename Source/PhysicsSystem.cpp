...
namespace physics
{
	PhysicsSystem::PhysicsSystem(component::EntityWorld* entityWorld)
	...
	void PhysicsSystem::start()
	{
		...
		for (auto& capsuleColliderComp : mEntityWorld->getCapsuleColliderWorld().getArray())
		{
			...
			else
			{
				...
				{
					component::RigidbodyComponent* rigidbodyComp = mEntityWorld->getRigidbodyWorld().getFirst(capsuleColliderComp.entityId);
	...
	void PhysicsSystem::update(float deltaTime)
	{
		...
		for (auto& charControllerComp : mEntityWorld->getCharacterControllerWorld().getArray())
		{
			...
			component::TransformComponent* transformComp = mEntityWorld->getTransformWorld().getFirst(charControllerComp.entityId);
			assert(transformComp);
		...
		while (mAccumulator >= mStepSize)
		{
			for (auto& rigidbodyComp : mEntityWorld->getRigidbodyWorld().getArray())
			{
				...
				component::TransformComponent* transformComp = mEntityWorld->getTransformWorld().getFirst(rigidbodyComp.entityId);
				assert(transformComp);
